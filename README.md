...
- **3D 翻转动效**：换图时随机使用水平翻转 / 垂直翻转 / 缩放淡入淡出，营造"活着的照片墙"
- **本地图片**：支持本地上传（持久化到 `data/uploads`）+ 自动扫描外部图库目录（默认 `/home/ma/图片`），混合轮播
- **网络图源**：可选开启 Picsum 真实网络图（默认关闭，离线可用本地渐变占位）
- **屏保效果**：基于 `requestAnimationFrame` 驱动，鼠标移出页面也持续自动换图
- **控制面板**：左侧吸附齿轮滑出玻璃风控制台，可调网格尺寸、切换模式、间隔、网络图占比、上传/删除本地图等，配置自动持久化到 localStorage
...
└── src/                    # React 前端
    ├── App.tsx             # 组装照片墙 + 控制面板，初始化换图引擎
    ├── components/         # Wall（网格）、WallCell（3D翻转卡片）、ControlPanel
...
| --- | --- | --- |
| `PORT` | 后端端口 | `3001` |
| `EXTERNAL_IMAGE_DIR` | 外部图库目录（启动时自动扫描其中的图片） | `/home/ma/图片` |
